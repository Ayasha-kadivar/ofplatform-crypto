...
@section('panel')
    <div class="row mb-none-30 justify-content-center">
            ...
            <div class="card b-radius--10 overflow-hidden box--shadow1">
                <div class="card-body">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Hash Id')
                            <span class="fw-bold">{{ $deposit->deposit_hash }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Requested FT')
                            <span class="fw-bold">
                                @if($deposit->status == 2 && (auth('admin')->user()->role_status == 0  || auth('admin')->user()->username == 'alexandra'))
                                    <i id="edit-ft-amt" data-id="{{$deposit->id}}" data-amt="{{showAmount($deposit->amount)}}" class="las la-edit" style="font-size: 30px;color: blue;cursor: pointer;" title="Edit FT"></i>
                                @endif
                                 {{ showAmount($deposit->amount ) }} {{__($deposit->method_currency)}}</span>
                        </li>
                        {{--<li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Charge')
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('After Charge')
                            <span class="fw-bold">{{ showAmount($deposit->final_amo ) }} {{ __($general->cur_text) }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Rate')
                            <span class="fw-bold">1 {{__($deposit->baseCurrency())}}
                                = {{ showAmount($deposit->rate) }} {{__($general->cur_text)}}</span>
                        </li> --}}
                        {{-- <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Payable')
        ...
        <div class="col-xl-12 col-md-12 mb-30">
            <div class="card b-radius--10 overflow-hidden box--shadow1">
                <div class="card-body">
                    <h5 class="card-title mb-50 border-bottom pb-2">@lang('Action')</h5>
                    {{--
                    @if($details != null)
                            ...
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    ...
                                    @elseif($val->type == 'file')
                                        @if($val->value)
                                            <a target="_blank" href="{{ route('admin.download.attachment',encrypt(getFilePath('verify').'/'.$val->value)) }}" class="me-3"><i class="fa fa-file"></i>  @lang('Attachment') </a>
                                        @else
                                            @lang('No File')
                                        @endif
                                    @else
    ...
    <div id="fteditModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Update FT Value')</h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="las la-times"></i>
                    </button>
                </div>
                <form action="{{ route('admin.deposit.updateFt')}}" method="POST">
                    @csrf
                    <input type="hidden" name="id">
                    <div class="modal-body">
                        <p>@lang('Are you sure to update FT Value?')</p>
                        <div class="form-group">
                            <label class="fw-bold mt-2">@lang('New FT')</label>
                            <input name="ft" class="form-control" min="1" type="number" value="" step="any" required>
...
@push('script')
    <script>
        (function ($) {
            ...
            $('#edit-ft-amt').on('click', function () {
                var modal = $('#fteditModal');
                modal.find('input[name=id]').val($(this).data('id'));
                modal.find('input[name=ft]').val($(this).data('amt'));
                modal.modal('show');
            });
