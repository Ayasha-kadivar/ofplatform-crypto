                    ...
                    <table class="table table--light style--two">
                        ...
                        @forelse($deposits as $deposit)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if(isset($deposit->user->fullname))
                                ...
                                <td>
                                   {{ __($deposit->method_currency) }} {{ showAmount($deposit->amount ) }} {{-- + <span class="text-danger" title="@lang('charge')">{{ showAmount($deposit->charge)}} </span>
                                     <br>
                                    <strong title="@lang('Amount with charge')">
                                    {{ showAmount($deposit->final_amo) }} {{ __($general->cur_text) }}
                                    </strong> --}}
                                </td>
                                {{-- <td>
                                   1 {{__($deposit->method_currency)}} =  {{ showAmount($deposit->rate) }} {{ __($general->cur_text) }}
                                    <br>
                                    <strong>{{ showAmount($deposit->final_amo) }} {{ __($general->cur_text) }}</strong>
                                </td> --}}
                                <td>
...
@push('breadcrumb-plugins')
    @if(!request()->routeIs('admin.users.deposits') && !request()->routeIs('admin.users.deposits.method'))
        <x-search-form dateSearch='yes' hashSearch="yes" placeholder="Search Hash Id" />
    @endif
@endpush
