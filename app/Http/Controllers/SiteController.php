...
// if ($user) {
//     if (is_null($user->created_at) && is_null($user->updated_at) && $user->deposit_wallet) {
//         $value = $user->deposit_wallet;
//         $divide_value = $value/4;
//         $user->update([
//             'created_at' => now(),
//             'updated_at' => now(),
//             'pool_1' => $divide_value,
...
//     } else {
//         $user->update([
//             'pool_1' => $user->pool_1 + $user->deposit_wallet,
//             'deposit_wallet' => 0,
//         ]);
//     }
// }
        $myPoolData = RentNFT::where('user_id',auth()->user()->id)->get()->last();
        ...
        $cube_to_wallet = WithdrawalsRequestCubeOneToWallet::where('status',1)->where('user_id',auth()->user()->id)->sum('amount');
        $final_iwallet_amount = $user->interest_wallet;
        if($cube_to_wallet){
            $final_iwallet_amount = ($final_iwallet_amount - $cube_to_wallet);
        }

        $ref_this_month = User::whereYear('created_at', Carbon::now()->year)
        ->whereMonth('created_at', Carbon::now()->month)
        ->where('ref_by',auth()->user()->id)
        ->count();
        
        return view($this->activeTemplate . $view, compact('cube_to_wallet', 'final_iwallet_amount', 'pageTitle', 'user', 'myPoolData', 'sections', 'layout', 'gatewayCurrency','ref_this_month'));
    }
