...
class GeneralSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $general->price_ft            = $request->price_ft;
        // dd($general->price_ft);
        $general->save();
    ...
    public function logoIconUpdate(Request $request)
    {
        $request->validate([
            ...
            'favicon' => ['image',new FileTypeValidate(['png','jpg','jpeg'])],
        ]);
        ...
        if ($request->hasFile('favicon')) {
            try {
                ...
                Image::make($request->favicon)->resize($size[0], $size[1])->save($path . '/favicon.jpeg');
            } catch (\Exception $exp) {
