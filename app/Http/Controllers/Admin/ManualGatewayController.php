...
class ManualGatewayController extends Controller
{
    ...
    public function update(Request $request, $code)
    {
        ...
        $method->name                 = $request->name;
        $method->alias                = strtolower(\Str::limit(\Str::slug($request->name),35));
        $method->gateway_parameters   = json_encode([]);
        $method->supported_currencies = [];
        $method->crypto               = 0;
        $method->description          = $request->instruction;
        ...
        $singleCurrency->gateway_alias  = strtolower(\Str::limit(\Str::slug($method->name),35));
        ...
        $singleCurrency->rate           = GeneralSetting::first()->price_ft;
        $singleCurrency->save();
