...
function getImage($image, $size = null)
{
    ...
    $exists = $disk->exists($image);
    if($exists){
        return $disk->url($image);
    }
    $fp = $image ? str_replace('assets/','',$image) : $image;
    $fexists = $disk->exists($fp);
    if($fexists){
        return $disk->url($fp);
    }
    if ($size) {
...
function fileUploader($file, $location, $size = null, $old = null, $thumb = null)
{
    ...
    $storeFile = $file->storeAs($location, $file_name, "gcs");
    $disk = \Storage::disk('gcs');
...
function getdays($req_val){
    return base64_decode($req_val);
}
...
function checkHashPayment($feeHash){
    // Check uniqueness in the users table
    $userExists = \DB::table('users')
    ->where('maintenance_fee_hash', $feeHash)
    ->exists();
